...
import { Search, Filter, Image as ImageIcon, Upload, CheckCircle, GraduationCap } from 'lucide-react';
...
const UpdatePhotos = () => {
    ...
    const handleFileChange = async (studentId, e) => {
        ...
        reader.onloadend = async () => {
            try {
                ...
                const res = await api.put(`/students/${studentId}/photo`, { photoUrl: base64String });
                
                if (res.data.success) {
                    toast.success("Photo updated successfully");
                    setStudents(prev => prev.map(s => s.id === studentId ? { ...s, student_photo: base64String } : s));
                }
            } catch (error) {
    ...
    return (
        <div className="p-2 anim-fade-in">
            <div className="mb-4">
                <h1 className="text-2xl font-bold text-primary-900" style={{ fontFamily: 'Times New Roman' }}>Update Student Photos</h1>
                <p className="text-[11px] text-gray-500 mt-0.5">Quickly view and update passport photos for students.</p>
            </div>
                    ...
                    <table className="w-full text-left text-sm whitespace-nowrap">
                        <thead className="bg-[#f8fafc] text-[#64748b] uppercase text-[9px] font-black tracking-wider border-b border-gray-100">
                            <tr>
                                ...
                                <th className="px-3 py-2">Current Photo</th>
                                <th className="px-3 py-2 w-64 text-right">Upload New Photo</th>
                            </tr>
                        </thead>
                            ...
                            {loading ? (
                                ...
                                students.map((student, index) => (
                                    <tr key={student.id} className="hover:bg-[#f8fafc]/50 transition-colors">
                                        ...
                                        <td className="px-3 py-3">
                                            <div className="w-12 h-14 bg-gray-100 rounded border border-gray-200 overflow-hidden flex items-center justify-center">
                                                {student.student_photo ? (
                                                    <img src={student.student_photo} alt="Student" className="w-full h-full object-cover" />
                                                ) : (
